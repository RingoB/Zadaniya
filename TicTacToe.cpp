...
int hod(int a, int b) {  //Ôóíêöèÿ, îòâå÷àşùàÿ çà õîäû.
	if (a == 1) {
		if (ryad1[b - 1] != '*') {
			return 1;
		}
...
int endgamecheck() { //Ïğîâåğêà êîìáèíàöèé
	int k = 0;
	int s = 0;
	int d = 0;
	int n = 0;
...
int main() { //Ñîáñòâåííî ìıèí.
	setlocale(LC_ALL, "rus");
newgame:
	cout << "Êğåñòèêè-íîëèêè v1.0 (ñ) RingoBarbos" << endl;
again:
	pole();
	int a, b;
	int hodi = 0;
	...
	char otvet;
retry:
	cout << "Õîä èãğîêà" << player << ": ";
	cin >> a >> b;
	if ((a > 10)||(b > 10) || (a < 1) || (b < 1)) {
		cout << "ÎØÈÁÊÀ! ÂÛ ÂÂÅËÈ ÊÎÎĞÄÈÍÀÒÛ ÇÀ ÏĞÅÄÅËÀÌÈ ÏÎËß!" << endl;
		goto retry;
	}
	check = hod(a,b);
	if (check == 1) {
...
zan:
	...
	if (end == 1) {
		pole();
		cout << "Èãğîê " << player << " ïîáåäèë!" << endl;
		cout << "Õîòèòå íà÷àòü çàíîâî? y/n?: ";
zanovo:
		cin >> otvet;
		if (otvet == 'y') {
